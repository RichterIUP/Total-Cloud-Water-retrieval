...
def read_input_cre(fname):
    dataset = nc.Dataset(fname)
    aux.FTIR = fname.split("/")[-1]
    aux.MONTH = aux.FTIR[15:17]
    aux.DAY = aux.FTIR[17:19]
    aux.YEAR = aux.FTIR[11:15]
    aux.HOUR = aux.FTIR[20:22]
    aux.MINUTE = aux.FTIR[22:24]
    aux.LAT = 0.0#dataset.variables['lat'][0]
    aux.LON = 0.0#dataset.variables['lon'][0]
    
    aux.WAVENUMBER_FTIR = np.arange(0, 3000, 0.3)#np.array(dataset.variables['wavenumber'][:])
    aux.RADIANCE_FTIR = np.zeros(len(aux.WAVENUMBER_FTIR))#np.array(dataset.variables['radiance'][:])+inp.OFFSET
    aux.NOISE_FTIR = np.zeros(len(aux.WAVENUMBER_FTIR))
    aux.SOLAR_ZENITH_ANGLE = 0.0#dataset.variables['sza'][0]   
    pressure = dataset.variables['pressure'][:]
    altitude = dataset.variables['height_level'][:]
    temperature = dataset.variables['temperature'][:]
    humidity = dataset.variables['rH'][:]
    inp.HUMIDITYY = "%"
    aux.ATMOSPHERIC_GRID = [pressure, altitude, temperature, humidity]
    inp.CLOUD_BASE = [-999]
    inp.CLOUD_TOP = [-999]
    
    aux.CLOUD_LAYERS = dataset.variables['cloud_level'][:]
    inp.FORWARD = True
    tl = np.float64(dataset.variables['tau_liq'][:])
    ti = np.float64(dataset.variables['tau_ice'][:])
    rl = np.float64(dataset.variables['rliq'][:])
    ri = np.float64(dataset.variables['rice'][:])
    inp.MCP = np.array([tl, ti, rl, ri])
