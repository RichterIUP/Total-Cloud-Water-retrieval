...
def jacobian():
    ...
    if inp.ONLY_OD:
        ...
        for ii in range(len(deriv_r_liq)):
            ...
            h.write("{},{},{}\n".format(aux.WAVENUMBER_FTIR[ii], aux.RADIANCE_LBLDIS[5][-1][ii], aux.RADIANCE_LBLDIS[0][-1][ii]))
            i.write("{},{},{}\n".format(aux.WAVENUMBER_FTIR[ii], aux.RADIANCE_LBLDIS[7][-1][ii], aux.RADIANCE_LBLDIS[0][-1][ii]))
        f.close()
        g.close()
        h.close()
        i.close()
