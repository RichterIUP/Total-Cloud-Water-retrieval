...
def __only_fwd(lblrtm=False):
    ...
    '''
    slope_lbldis = (aux.RADIANCE_LBLDIS[0][-1][0] - aux.RADIANCE_LBLDIS[0][-1][14])/(aux.WAVENUMBER_FTIR[0]-aux.WAVENUMBER_FTIR[14])
    slope_ftir = (aux.RADIANCE_FTIR[0] - aux.RADIANCE_FTIR[14])/(aux.WAVENUMBER_FTIR[0]-aux.WAVENUMBER_FTIR[14])
    '''
    f.write("{}\n".format(slope_lbldis))
    f.write("{}\n".format(slope_ftir))
    f.close()
